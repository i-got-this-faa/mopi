        ...
        Some("roots") => match args.get(2).map(String::as_str) {
            Some("add") => {
                if let Some(new_root) = args.get(3) {
                    ...
                    println!("usage: mopictl roots add <path>");
                }
            }
            Some("remove") => {
                if let Some(target_root) = args.get(3) {
                    ...
                    println!("usage: mopictl roots remove <path>");
                }
            }
            _ => {
...
async fn send_request(paths: &MopiPaths, request: Request) -> Result<Response> {
    let config = AppConfig::load_or_default(paths)?;
...
fn print_usage(paths: &MopiPaths) {
    println!(
        "usage: mopictl <init-config|config show|config validate|ping|status|stats|roots|reload-config|refresh|doctor|search ...>\nconfig={}\nsocket={}",
        paths.config_file,
        paths.socket_file()
    );
}
...
async fn add_root(paths: &MopiPaths, new_path: &str) -> Result<()> {
    if !paths.config_file.exists() {
        AppConfig::write_default(paths)?;
    }
...
async fn remove_root(paths: &MopiPaths, target_path: &str) -> Result<()> {
    if !paths.config_file.exists() {
        return Ok(());
    }
